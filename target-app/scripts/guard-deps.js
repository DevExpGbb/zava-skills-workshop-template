#!/usr/bin/env node
/**
 * Preinstall guard — prevents `target-app/` from accidentally pulling in
 * the deliberately-vulnerable packages that live (intentionally) under
 * `target-app/security-fixtures/`.
 *
 ...
 *
 * Banned in `target-app/package.json` (the app):
 *   - lodash           (use lodash-es or scoped utilities)
 *   - axios <  1       (CVE backlog)
 *   - minimist < 1.2.6 (prototype pollution)
 *
 * These are still allowed under `target-app/security-fixtures/` — that's
 * the audit target.
 */
...
if (violations.length) {
  console.error('\n✋ target-app preinstall guard tripped:\n');
  console.error(violations.join('\n'));
  console.error('\nIf you meant to install/upgrade in the audit fixture, use:');
  console.error('    npm install --prefix target-app/security-fixtures <pkg>@<version>');
